...
export default function Dashboard() {
  ...
  const { budgets, spending, currentMonth, getCategoryStatus, getTotalSpent, getTotalBudgeted } = useBudgetStore()
  ...
  const underBudget = categories.filter(cat => {
    const { status } = getCategoryStatus(cat)
    return status === 'under'
  }).length
    ...
    <MainLayout>
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="text-body-md text-on-surface-variant mt-2">September 2026</p>
        </div>
        ...
        <div className="bg-gradient-to-r from-primary to-primary bg-opacity-10 rounded-lg p-8 border border-primary border-opacity-20">
          <p className="text-label-md text-on-surface-variant uppercase tracking-wide">Total Net Worth</p>
          <p className="text-5xl font-bold text-primary mt-2">${totalAssets.toFixed(2)}</p>
          <p className="text-body-sm text-on-surface-variant mt-3">Across all accounts</p>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {accounts.map(account => (
              <div key={account.id} className="bg-surface-container rounded-lg p-6 border border-border-hairline hover:border-primary transition-colors">
                <p className="text-label-md text-on-surface-variant">{account.name}</p>
                <p className="text-headline-md font-bold text-on-surface mt-2">${account.balance.toFixed(2)}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-surface-container rounded-lg p-6 border border-border-hairline">
              ...
              <p className="text-headline-md font-bold mt-2">
                <span className={budgetHealth > 100 ? 'text-status-error' : budgetHealth > 80 ? 'text-status-warning' : 'text-status-success'}>
                  {budgetHealth}%
                </span>
              </p>
              <div className="w-full bg-surface rounded-full h-2 mt-3 overflow-hidden">
                <div
                  className={`h-full transition-all ${
                    budgetHealth > 100 ? 'bg-status-error' :
                    budgetHealth > 80 ? 'bg-status-warning' :
                    'bg-status-success'
                  }`}
            ...
            <div className="bg-surface-container rounded-lg p-6 border border-border-hairline">
              ...
              <p className="text-headline-md font-bold text-on-surface mt-2">${totalBudgeted.toFixed(2)}</p>
            ...
            <div className="bg-surface-container rounded-lg p-6 border border-border-hairline">
              ...
              <p className="text-headline-md font-bold text-on-surface mt-2">${totalSpent.toFixed(2)}</p>
              <p className="text-body-sm text-on-surface-variant mt-1">This month</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-surface-container rounded-lg p-6 border border-border-hairline">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className={`text-headline-md font-bold ${overBudget > 0 ? 'text-status-error' : 'text-status-success'}`}>
                  {overBudget}
                </span>
              </div>
              <div className="space-y-2">
        ...
        <div className="flex gap-4">
          <button className="flex-1 py-3 bg-primary text-surface rounded font-medium hover:opacity-90 transition-opacity">
            + Add Transaction
          </button>
          <button className="flex-1 py-3 bg-surface-container text-on-surface border border-border-hairline rounded font-medium hover:bg-surface-container-high transition-colors">
            View Transactions
          </button>
        </div>
      </div>
