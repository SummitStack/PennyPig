-- Fixed budget parents (Needs / Wants / Savings Goals / Other) + remount groups.

ALTER TABLE public.categories
  ADD COLUMN IF NOT EXISTS is_system boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.categories.is_system IS
  'System parents (Needs/Wants/Savings Goals/Other) cannot be renamed, moved, or deleted.';
...
BEGIN
  ...
  -- If Savings Goals existed as a non-system group first, the upsert set is_system=true on it.
  ...
  -- ── Groups under parents (legacy names preserved: Living, Food & Dining, …) ─
  INSERT INTO public.categories (user_id, name, type, color, icon, custom, sort_order, parent_id, is_system)
  VALUES
  ...
  ON CONFLICT (user_id, name) DO UPDATE
    SET icon = COALESCE(NULLIF(public.categories.icon, ''), EXCLUDED.icon),
        sort_order = EXCLUDED.sort_order,
        type = EXCLUDED.type,
        -- Remount legacy top-level groups under the locked parents when still root/misplaced
        parent_id = CASE
  ...
  -- If an old Savings Goals group had children before it became the parent,
  -- those children should hang under the Savings Goals parent as categories
  -- (or stay if already correct). No separate middle group needed.
  ...
  -- Remount any leftover top-level expense categories (non-system) into Other
  ...
  WHERE user_id = uid
    ...
    AND is_system = false
    AND name NOT IN ('Needs', 'Wants', 'Savings Goals', 'Other', 'Salary');

  -- Ensure system parents stay roots
  UPDATE public.categories
